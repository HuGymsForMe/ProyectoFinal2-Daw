...
function UpdateUserPage() {
    ...
    useSeo({
        title: "Actualizar Usuario | AutoescuelaFast",
        description: "Actualiza tu usuario para poder acceder a nuestra aplicación"
    })
    ...
    const { isAuthenticated, errors: updateErrors, user } = useAuth();
        ...
        <main className="flex justify-center items-center min-h-screen">
            <section className="bg-white p-8 flex flex-col gap-y-8 rounded-xl shadow-slate-500 shadow-lg z-10 sm:w-auto w-[90%] md:my-auto my-16">
                <div className="flex gap-x-10 gap-y-4 md:flex-row flex-col justify-center items-center">
                    <h1 className={`text-4xl text-[#C21D30]`}>Editar Perfil</h1>
                ...
                <form className="flex flex-col gap-x-4" onSubmit={handleSubmit(onSubmit)}>
                        ...
                        <div className="flex flex-col gap-y-4 m-2">
                            <div className="flex flex-col">
                                ...
                                <input type="text" placeholder="Nombre" className="rounded-md p-2 bg-slate-200" 
                                    {...register("name", { required: true })} value={user.name} disabled />
                                {errors.name?.message && (
                            ...
                            <div className="flex flex-col">
                                ...
                                <input type="text" placeholder="Apellidos" className="rounded-md p-2 bg-slate-200"
                                    {...register("surnames", { required: true })} value={user.surnames} disabled  />
                            ...
                            <div className="flex flex-col">
                                ...
                                <input type="text" placeholder="Nombre de usuario" className="rounded-md p-2 bg-slate-200"
                                    {...register("username", { required: true })} value={user.username} disabled />
                        ...
                        <div className="flex flex-col gap-y-4 m-2">
                            <div className="flex flex-col">
                                ...
                                <input type={seePassword ? "text" : "password"} placeholder="Contraseña" className="rounded-md p-2 bg-slate-200"
                                    {...register("password", { required: true })} value={user.password} />
                            ...
                            <div className="flex flex-col">
                                ...
                                <input type={seePasswordConfirm ? "text" : "password"} placeholder="Confirmar Contraseña" className="rounded-md p-2 bg-slate-200"
                                    {...register("confirm_password", { required: true })} value={user.password} />
                            ...
                            <div className="flex flex-col">
                                ...
                                <input type="date" placeholder="Fecha de Nacimiento" className="rounded-md p-2 bg-slate-200"
                                    {...register("birthday", { required: true })} value={user.birthday} disabled />
                    ...
                    <div className="flex justify-around md:mt-0 mt-8 sm:flex-row flex-col">
                        <button className={`bg-[#C21D30] text-white px-8 py-2 rounded-md cursor-pointer hover:shadow-xl hover:bg-[#B30519] `}>Actualizar Perfil</button>
                    </div>
                </form>
            </section>
            {showToast && !passwordsEquals && (<ToastErrors onClose={closeToast} error={true}>Las dos contraseñas son distintas.</ToastErrors>)}
            {showToast && errorHTTP && (<ToastErrors onClose={closeToast}>{updateErrors}</ToastErrors>)}
        </main>
        <Footer />
        </>
    )
