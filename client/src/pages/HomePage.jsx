        ...
        <>
            ...
            <main className="text-white py-8 gap-y-8 flex flex-col items-center">
                ...
                <section className="flex xl:flex-row flex-col w-[75%] items-center gap-6">
                    <Link to={isAuthenticated ? `/tests/${user.id}` : "/login"}>
                    ...
                    <Link to={isAuthenticated ? `/profile/${user.id}` : "/login"}>
                        <div className="flex flex-col items-center px-4 py-8 gap-y-4 rounded-xl bg-white xl:w-[50%] w-[100%]">
                            ...
                            <p className={`text-[#C21D30] lg:text-center text-pretty`}>¡ Podrás ver toda tu evolución ! Cuántas veces has entrado, qué videos has visualizado, cuántos y qué tests has hecho y qué fallos has tenido para que puedas repasar. ¡ Además te damos tu probabilidad de aprobado !</p>
                        </div>
                    </Link>
                </section>
